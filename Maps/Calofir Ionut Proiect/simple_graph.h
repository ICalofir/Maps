...
void simple_graph::operator = (const simple_graph &ob)
{
...
int simple_graph::isEqual(char *sir)
{
	int lg = strlen(sir);
...
void simple_graph::bellman(int nodInit)
{
    myVector Q;
    int first = 1;
    Q.push_back(nodInit);
...
void simple_graph::addS(char *sir)
{
	int lg = strlen(sir);
...
void simple_graph::add_citiesR(int A, int B)
{
    bool ok = true;
...
int simple_graph::get_minR(int A, int B)
{
    return D[A][B];
}

void simple_graph::actualize_dist() // 1 - linie, 2 - arbore, 3 - dag, 4 - graf normal, 5 - graf complet
{
    for (int i = 1; i <= get_cities_size::cities_size(); ++i)
    {
        for (int j = 1; j <= get_cities_size::cities_size(); ++j)
